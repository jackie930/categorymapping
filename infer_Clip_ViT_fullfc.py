...
class myDataset(Dataset):
    ...
    def get_inputs(self, idx):
        # Tokenize sentences
        encoded_input1 = self.tokenizer(self.input_data[idx]['text1'], padding=True, truncation=True,
                                        return_tensors='pt')
        text_output1 = torch.nn.functional.normalize(self.emb_model(**encoded_input1)[0][:, 0], p=2, dim=1)

        encoded_input2 = self.tokenizer(self.input_data[idx]['text2'], padding=True, truncation=True,
                                        return_tensors='pt')
...
def infer(args):
    ...
    model.eval()
    ss = []
    for ids, text1, text2, image1, image2, label in tqdm(data_loader_test):
        score = model(text1, text2, image1, image2)
        score = accelerator.gather_for_metrics(score.item())
        ss.append([ids[0], score, label.cpu().numpy()[0]])
